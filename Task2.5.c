...
void safeZeroTurn(float degrees) {
...
void moveStack(int start, int end, int number)
{
    ...
    if(number == 1) {
        //Move the robot to visualize algorithm.
        safeMoveForward((start-robotPos)*scale);
        towers[start]--;
        robotPos = start;
        safeZeroTurn(360.0F);
        safeMoveForward((end-robotPos)*scale);
        towers[end]++;
        safeZeroTurn(360.0F);
        robotPos = end;
        printTower();
...
int main()
{
    ...
    safeMoveForward((1.5-robotPos)*scale);
    safeZeroTurn(-90);
}
